    ...
    <div class="product-top-layout">
        ...
        <div class="image-section">
            <div class="thumbnail-list">
            	<c:forEach var="image" items="${pdto.imageDTOList}">
                <img id="mainProductImage" src="${image.image_url}" class="thumb-img active" alt="${pdto.productDTO.product_name}">
                </c:forEach>
            </div>
            <div class="main-image-wrap">
                ...
                <img src="${pdto.imageDTOList[0].image_url}" alt="${pdto.productDTO.product_name}">
                </c:if>
            </div>
        </div>
        ...
        <div class="info-section">
            <div class="brand-row">
                <span class="brand-name">${pdto.productDTO.brand_name}</span>
            ...
            <h1 class="title product-name">${pdto.productDTO.product_name}</h1>
            ...
            <div class="original-price-row">
                <span class="discount-rate"><fmt:formatNumber value="${pdto.productDTO.discount_rate}"/>%</span>
                <span class="original-price"><fmt:formatNumber value="${pdto.productDTO.original_price}" pattern="#,###"/>원</span>
            </div>
            
            <div class="current-price-row">
                <span class="current-price"><fmt:formatNumber value="${pdto.productDTO.price}" pattern="#,###"/>원</span>
         ...
         <div class="option-area">
			<!-- 필수/추가 옵션 선택 영역 -->
            <select class="option-select" data-group_id="10" data-group_name="사이즈" data-required="1">
                <option value="" disabled selected>사이즈</option>
	              <c:forEach var="option" items="${pdto.optionDTOList}">
		             <c:if test="${option.option_group_id == 10}">     
		                <option value="${option.option_value_id}">${option.option_name}</option>
		             </c:if>   
	              </c:forEach>  
            </select>

            <select class="option-select" data-group_id="11" data-group_name="색상" data-required="1">
                <option value="" disabled selected>색상</option>
	              <c:forEach var="option" items="${pdto.optionDTOList}">
		             <c:if test="${option.option_group_id == 11}">     
		                <option value="${option.option_value_id}">${option.option_name}</option>
		             </c:if>   
	              </c:forEach>  
            </select>
            
            <select class="option-select" data-group_id="12" data-group_name="추가상품(선택)" data-required="0">
                <option value="" selected>추가상품(선택)</option>
	              <c:forEach var="option" items="${pdto.optionDTOList}">
		             <c:if test="${option.option_group_id == 12}">     
		                <option value="${option.option_value_id}">${option.option_name}</option>
		             </c:if>   
	              </c:forEach>  
            </select>
            ...
            <!-- 필수 옵션 경고 -->
            <div id="optionWarning">
            	필수 옵션을 선택해주세요.
            </div>
            
            <!-- 선택된 옵션이 js에 의해 들어가는 곳 -->
            <div id="selectedList" class="selected-list"></div>
         </div>
            <div id="optionToast" class="option-toast" style="display: none;"></div>
        </div>
    </div>
