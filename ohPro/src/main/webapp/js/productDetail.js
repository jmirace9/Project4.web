...
selects.forEach(function (select) {
    select.addEventListener("change", function () {
        ...
        if (this.dataset.required === "1") {
			...
			console.log("현재 선택:", this);
			console.log("필수 옵션 개수:", required_selects.length);
			console.log("현재 index:", current_index);
			console.log("다음 옵션:", next_required);

			if (next_required) {
			    next_required.disabled = false;
			    console.log("다음 옵션 disabled:", next_required.disabled);
			}
...
function findProductOption() {
    ...
    const optional_selects = selects.filter(function (select) {
    ...
    if (all_required_selected) {
        ...
        fetch(
			contextPath + "/shopping/product/productOption.htm?product_id=" +
    ...
    /* 추가 옵션 */
    optional_selects.forEach(function (select) {
        if (select.value === "") {
            return;
        }

        const option_value_id = select.value;
